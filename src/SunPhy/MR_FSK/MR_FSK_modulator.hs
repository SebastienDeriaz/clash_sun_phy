module SunPhy.MR_FSK.MR_FSK_modulator where

import Clash.Prelude

import SunPhy.Concat2 (concat2)
import SunPhy.Concat3 (concat3)
import SunPhy.MR_FSK.FEC (fec)
import SunPhy.MR_FSK.Interleaver (interleaver)
import SunPhy.MR_FSK.PHR (phr)
import SunPhy.MR_FSK.PSDU (psdu)
import SunPhy.MR_FSK.SHR (shr)
import SunPhy.Scrambler (scrambler)
import SunPhy.Splitter2 (splitter2)
...
mrFskModulator
    ...
    psdu_last_i =
      ...
      where
        ...
        (fec_ready_i, interleaver_data_o, interleaver_valid_o, interleaver_last_o) =
            unbundle $
                interleaver
                    interleaver_bypass
                    fec_valid_o
                    ...
                    interleaver_ready_i
        -- Splitter 2
        (interleaver_ready_i, splitter2_a_valid_o, splitter2_a_data_o, splitter2_a_last_o, splitter2_b_valid_o, splitter2_b_data_o, splitter2_b_last_o) =
            unbundle $
                splitter2
                    interleaver_valid_o
                    interleaver_data_o
