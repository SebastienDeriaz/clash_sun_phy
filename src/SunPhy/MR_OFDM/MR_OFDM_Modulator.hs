...
mrOfdmModulator
  ...
  -> Signal dom (Bit, Bit, IQ, Bit, Bit, Bit, Bit, Bit, Bit, Bit, Unsigned 4) -- PSDU, output, psdu_ready_o
mrOfdmModulator
  -- Inputs
  ofdmOption
  mcs
  ...
  where
    ...
    -- 4) OFDM
    (psdu_interleaver_ready_i, psdu_ofdm_valid_o, psdu_ofdm_data_o, psdu_ofdm_last_o, _, _, _) = unbundle $ ofdm
      ofdmOption
      mcs
      (pure 1)
    ...
    (phr_ready_i, phr_encoder_valid_o, phr_encoder_data_o, phr_encoder_last_o) = unbundle $ encoder
      ...
      phr_encoder_ready_i
    -- 3) Interleaver
    (phr_encoder_ready_i, phr_interleaver_valid_o, phr_interleaver_data_o, phr_interleaver_last_o) = unbundle $ interleaver
      (pure 0)
      lowest_mcs
      ofdmOption
      phyOFDMInterleaving
      ...
      phr_interleaver_ready_i
    -- 4) OFDM
    (phr_interleaver_ready_i, phr_ofdm_valid_o, phr_ofdm_data_o, phr_ofdm_last_o, phr_pilotset, pn9_seed, ofdm_state) = unbundle $ ofdm
      ofdmOption
      mcs
      (pure 1)
      phr_interleaver_valid_o
    ...
    -- Concatenate
    (stf_ready_i, ltf_ready_i, phr_ofdm_ready_i, psdu_ofdm_ready_i, valid_o, data_o, last_o, state_num) = unbundle $ concat4
      stf_valid_o
      stf_data_o
      stf_last_o
      ltf_valid_o
